/////// Updated for EHJ revision
// 1. adding endpoints: total stroke, hemorrhagic stroke (HS), any stroke(I60-61, I63-64), AND HS+IS(I61,I63)  

////// This code was used to calculated the cause of death. 更改了原本的数据库
...
***Endpoints
import delimited "D:\HanYT\00-原始数据\2021-02 Multimorbidity\endpoints.csv", encoding(utf8) clear
rename csid studyid
...
save "D:\HanYT\2021-02 CMM to death\1. Data\endpoints.dta", replace

***Baseline
import delimited "D:\HanYT\00-原始数据\2021-02 Multimorbidity\data_baseline_questionnaires.csv", encoding(utf8) clear
rename csid studyid
save "D:\HanYT\2021-02 CMM to death\1. Data\baseline.dta", replace

***Merge
use "D:\HanYT\2021-02 CMM to death\1. Data\baseline.dta", clear

* 收集所有变量
merge 1:1 studyid using "D:\HanYT\2021-02 CMM to death\1. Data\endpoints.dta", keep(match) nogen keepusing($eplist_status $eplist_date)
...
misstable summarize $eplist_status $eplist_status c_ep0101 c_ep0101_date du_ep9999 du_ep9999_date c_ep0088 c_ep0011 c_ep0088_date c_ep0011_date
save "D:\HanYT\2021-02 CMM to death\1. Data\merged_database.dta",replace

**********************************************************************************************************
****************************           Part 1.2 Baseline endpoints    ************************************
**********************************************************************************************************
*** Import dataset
use "D:\HanYT\2021-02 CMM to death\1. Data\merged_database.dta",clear
...
* IS + HS
egen d_number_inc1 = rowtotal(c_ep0003 c_ep0011 c_ep0088)


* Any stroke
egen d_number_inc2 = rowtotal(c_ep0003 c_ep0010 c_ep0088)

* IS + HS (四种疾病转换)
egen d_number_inc3 = rowtotal(c_ep0003 c_ep0008 c_ep0009 c_ep0088)
...
* IS+HS
recode d_number_inc1 (0/1=0 "None") (2/3=1 "multimorbidity"), gen(mltmbd_inc1)
egen mltmbd_inc_date1 = rowmedian(c_ep0003_date c_ep0011_date c_ep0088_date) if mltmbd_inc1 == 1
	replace mltmbd_inc_date1 = max(c_ep0003_date, c_ep0011_date, c_ep0088_date) if mltmbd_inc1 == 0
	format mltmbd_inc_date1 %td
* Any stroke
recode d_number_inc2 (0/1=0 "None") (2/3=1 "multimorbidity"), gen(mltmbd_inc2)
egen mltmbd_inc_date2 = rowmedian(c_ep0003_date c_ep0010_date c_ep0088_date) if mltmbd_inc2 == 1
	replace mltmbd_inc_date2 = max(c_ep0003_date, c_ep0010_date, c_ep0088_date) if mltmbd_inc2 == 0
	format mltmbd_inc_date2 %td

* IS+HS (四种疾病转换)
recode d_number_inc3 (0/1=0 "None") (2/max=1 "multimorbidity"), gen(mltmbd_inc3)
rowsort c_ep0003_date c_ep0008_date c_ep0009_date c_ep0088_date, gen(x1-x4)
gen mltmbd_inc_date3 = max(c_ep0003_date, c_ep0008_date, c_ep0009_date, c_ep0088_date) if mltmbd_inc3 == 0
	replace mltmbd_inc_date3 = x2 if mltmbd_inc3 == 1
	format mltmbd_inc_date3 %td
...
* IS + HS
gen first_cmd_inc1 = 1 if c_ep0003 == 1 | c_ep0088 == 1 | c_ep0011 == 1
	replace first_cmd_inc1 = 0 if mi(first_cmd_inc1)
egen first_cmd_inc_date1 = rowmin(c_ep0003_date c_ep0088_date c_ep0011_date)
gen first_cmd_inc3 = first_cmd_inc1
egen first_cmd_inc_date3 = rowmin(c_ep0003_date c_ep0088_date c_ep0009_date  c_ep0008_date)
* Any stroke
gen first_cmd_inc2 = 1 if c_ep0003 == 1 | c_ep0088 == 1 | c_ep0010 == 1
	replace first_cmd_inc2 = 0 if mi(first_cmd_inc2)
egen first_cmd_inc_date2 = rowmin(c_ep0003_date c_ep0088_date c_ep0010_date)

format first_cmd_inc_date1 first_cmd_inc_date2 first_cmd_inc_date3 %td
...
/* Parental history of cardiomebolic multimorbidity */
***双亲家族史
...
gen double death_type = 1 if first_cmd_inc1 == 0 & mltmbd_inc1 == 0
	replace death_type = 2 if first_cmd_inc1 == 1 & mltmbd_inc1 == 0
	replace death_type = 3 if first_cmd_inc1 == 1 & mltmbd_inc1 == 1

tab death_type, missing

egen total_death = rowtotal(du_ep0002 du_ep0014 du_ep0032 du_ep9999)

total du_ep0002 du_ep0014 du_ep0032 du_ep9999 if death_type == 1 
total du_ep0002 du_ep0014 du_ep0032 du_ep9999 if death_type == 2 
total du_ep0002 du_ep0014 du_ep0032 du_ep9999 if death_type == 3 

gen cause_death = 1 if du_ep0002 == 1
	replace cause_death = 2 if du_ep0014 == 1
	replace cause_death = 3 if du_ep0032 == 1
	replace cause_death = 4 if du_ep9999 == 1
label define cause_death 1 "Vascular" 2 "Cancer" 3 "Respiratory" 4 "Others"
label values cause_death cause_death
