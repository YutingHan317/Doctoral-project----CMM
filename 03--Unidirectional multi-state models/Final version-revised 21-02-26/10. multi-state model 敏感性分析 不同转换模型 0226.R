...
project4.trans$first_cmd_inc_date <- as.numeric(project4.trans$first_cmd_inc_date1)
project4.trans$mltmbd_inc_date <- as.numeric(project4.trans$mltmbd_inc_date1)
...
project4.trans$first_cmd_inc <- as.numeric(project4.trans$first_cmd_inc1)
project4.trans$mltmbd_inc <- as.numeric(project4.trans$mltmbd_inc1)
...
long_lf <- c("healthy_smoking","healthy_alcohol","healthy_diet","healthy_bmi","healthy_WC","healthy_sleep","healthy_PA","hscore","hscore.cat",
             "risky_smoking","risky_alcohol","risky_diet","risky_bmi","risky_WC","risky_sleep","risky_PA","risky_obesity","rscore","rscore.cat")
...
single2.trans <- coxph(Surv(Tstart, Tstop, status) ~ strata(trans)+ strata(region_code) + strata(age_strata) +
                     ...
                     risky_obesity.1 + risky_obesity.2  + risky_obesity.3 + risky_obesity.4 + risky_obesity.5 + risky_obesity.6,
                 data = mltstate.final.trans, method = "breslow")
single.hr2.trans <- data.frame(round(summary(single2.trans)$conf.int,2))
single.hr2.trans[,5] <- paste0(sprintf(single.hr2.trans[,1],fmt="%.2f")," (",sprintf(single.hr2.trans[,3],fmt="%.2f"),"-",sprintf(single.hr2.trans[,4],fmt="%.2f"),")")
...
scorecat.hr2.trans[,5] <- paste0(sprintf(scorecat.hr2.trans[,1],fmt="%.2f")," (",sprintf(scorecat.hr2.trans[,3],fmt="%.2f"),"-",sprintf(scorecat.hr2.trans[,4],fmt="%.2f"),")")
...
scoreord.hr2.trans[,5] <- paste0(sprintf(scoreord.hr2.trans[,1],fmt="%.2f")," (",sprintf(scoreord.hr2.trans[,3],fmt="%.2f"),"-",sprintf(scoreord.hr2.trans[,4],fmt="%.2f"),")")
