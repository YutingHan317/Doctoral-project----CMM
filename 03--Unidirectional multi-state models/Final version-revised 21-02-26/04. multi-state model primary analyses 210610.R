...
project4$first_cmd_inc_date <- as.numeric(project4$first_cmd_inc_date1)
project4$mltmbd_inc_date <- as.numeric(project4$mltmbd_inc_date1)
project4$du_ep0001_date <- as.numeric(project4$du_ep0001_date)
project4$dob_anon <- as.numeric(project4$dob_anon)

# 改变纳入CMD的范围
# disease status直接替换
project4$first_cmd_inc <- as.numeric(project4$first_cmd_inc1)
project4$mltmbd_inc <- as.numeric(project4$mltmbd_inc1)
...
ddply(mltstate.final,c("trans","rscore.cat")
  ,summarise,a=sum(status))
...
ddply(mltstate.final,c("trans","education_3groups"),summarise,a=sum(status))
# Incidence rate
ddply(mltstate.final,"trans",summarise,a=sum(status)/sum(time)*10000)
ddply(mltstate.final,"trans",summarise,a=sum(status)/sum(time)*10000,b=sum(status),c=sum(time))


##############################################
# Comment 提出的意见
# Transition Pattern A
total <- ddply(mltstate.final,c("trans"),summarise,a1=sum(status),b1=sum(time),c1=sum(status)/sum(time)*10000)
total[,5] <- "total"
colnames(total)[5] <- "name"
total.a <- dcast(total,name~trans,value.var = "a1")
total.b <- dcast(total,name~trans,value.var = "c1")

temp1 <- ddply(mltstate.final,c("trans","risky_smoking"),summarise,a1=sum(status),b1=sum(time),c1=sum(status)/sum(time)*10000)
colnames(temp1)[2] <- "name"
temp1.a <- dcast(temp1,name~trans,value.var = "a1")
temp1.b <- dcast(temp1,name~trans,value.var = "c1")

temp2 <- ddply(mltstate.final,c("trans","risky_alcohol"),summarise,a2=sum(status),b2=sum(time),c2=sum(status)/sum(time)*10000)
colnames(temp2)[2] <- "name"
temp2.a <- dcast(temp2,name~trans,value.var = "a2")
temp2.b <- dcast(temp2,name~trans,value.var = "c2")

temp3 <- ddply(mltstate.final,c("trans","risky_diet"),summarise,a3=sum(status),b3=sum(time),c3=sum(status)/sum(time)*10000)
colnames(temp3)[2] <- "name"
temp3.a <- dcast(temp3,name~trans,value.var = "a3")
temp3.b <- dcast(temp3,name~trans,value.var = "c3")

temp4 <- ddply(mltstate.final,c("trans","risky_obesity"),summarise,a4=sum(status),b4=sum(time),c4=sum(status)/sum(time)*10000)
colnames(temp4)[2] <- "name"
temp4.a <- dcast(temp4,name~trans,value.var = "a4")
temp4.b <- dcast(temp4,name~trans,value.var = "c4")

temp5 <- ddply(mltstate.final,c("trans","risky_PA"),summarise,a5=sum(status),b5=sum(time),c5=sum(status)/sum(time)*10000)
colnames(temp5)[2] <- "name"
temp5.a <- dcast(temp5,name~trans,value.var = "a5")
temp5.b <- dcast(temp5,name~trans,value.var = "c5")

temp6 <- ddply(mltstate.final,c("trans","rscore.cat"),summarise,a6=sum(status),b6=sum(time),c6=sum(status)/sum(time)*10000)
...
write.csv(a, "D:/HanYT/2019-06 Multimorbidity/3. Result/multistate/For comment/transition pattern A No. case.csv")
b <- rbind(temp1.b,temp2.b,temp3.b,temp4.b,temp5.b,temp6.b,total.b)
write.csv(b, "D:/HanYT/2019-06 Multimorbidity/3. Result/multistate/For comment/transition pattern A incidence.csv")
