...
**********************************************************************************************************
**********************************************************************************************************
log using "D:\HanYT\2021-02 CMM to death\2. Result\Log\data cleaning multi to death V1.log",replace
**********************************************************************************************************
...
*** duration groups
egen duration_chd_groups = cut(duration_chd), at(0 1 2(2)30 90) icodes
	replace duration_chd_groups = 99 if duration_chd ==.
egen duration_stroke_groups = cut(duration_stroke), at(0 1 2(2)30 90) icodes
	replace duration_stroke_groups = 99 if duration_stroke ==.
egen duration_diabetes_groups = cut(duration_diabetes), at(0 1 2(2)30 90) icodes
...
save "D:\HanYT\2021-02 CMM to death\1. Data\basic database.dta",replace
...
**********************************************************************************************************
******Exclusion
use "D:\HanYT\2021-02 CMM to death\1. Data\basic database.dta", clear
drop if mi(bmi_calc)
...
save "D:\HanYT\2021-02 CMM to death\1. Data\project2.dta",replace
...
foreach e of local elist{
	...
	*** Updated single CMD   2021-04-03
	...
	gen stroke_diag_updated = 0 if stroke_or_tia_diag == 0 & c_ep0003 == 0
		replace stroke_diag_updated = 1 if stroke_or_tia_diag == 1 
		replace stroke_diag_updated = 1 if stroke_or_tia_diag == 0 & c_ep0003 == 1 & age_stroke_diag >= _t0 & age_stroke_diag < _t
		replace stroke_diag_updated = 1 if stroke_or_tia_diag == 0 & c_ep0003 == 1 & age_stroke_diag < _t0 
		replace stroke_diag_updated = 0 if stroke_or_tia_diag == 0 & c_ep0003 == 1 & age_stroke_diag >= _t
		label variable stroke_diag_updated "updated stroke status"
		label values stroke_diag_updated baseline_disease

	gen diabetes_diag_updated = 0 if has_diabetes == 0 & c_ep0003 == 0
		replace diabetes_diag_updated = 1 if has_diabetes == 1 
		replace diabetes_diag_updated = 1 if has_diabetes == 0 & c_ep0003 == 1 & age_diabetes_diag >= _t0 & age_diabetes_diag < _t
		replace diabetes_diag_updated = 1 if has_diabetes == 0 & c_ep0003 == 1 & age_diabetes_diag < _t0
		replace diabetes_diag_updated = 0 if has_diabetes == 0 & c_ep0003 == 1 & age_diabetes_diag >= _t
	...
	*** Updated combinatioin of CMDs
	gen combin_updated = 0 if diabetes_diag_updated == 0 & chd_diag_updated == 0 & stroke_diag_updated == 0
		...
		label variable combin_updated "Baseline Combination"
	...
	*** Description
	...
	save "D:\HanYT\2021-02 CMM to death\1. Data\project2_updated_`e'.dta",replace
...
foreach e of local elist{
	...
	use "D:\HanYT\2021-02 CMM to death\1. Data\project2_updated_`e'.dta", clear
	...
	*** duration groups
	egen duration_chd_updated_groups = cut(duration_chd_updated), at(0 1 2(2)30 90) label
		replace duration_chd_updated_groups = 99 if duration_chd_updated ==.
	egen duration_stroke_updated_groups = cut(duration_stroke_updated), at(0 1 2(2)30 90) label
		replace duration_stroke_updated_groups = 99 if duration_stroke_updated ==.
	egen duration_diabetes_updated_groups = cut(duration_diabetes_updated), at(0 1 2(2)30 90) label
		replace duration_diabetes_updated_groups = 99 if duration_diabetes_updated ==.
	save "D:\HanYT\2021-02 CMM to death\1. Data\project2_updated_`e'_duration.dta", replace 
