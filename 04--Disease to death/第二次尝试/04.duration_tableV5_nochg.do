...
/// Version 5
* 2021/11/29 adding interaction with Updated CMM
...
*** stcox
stcox i.`exposure' i.`exposure'#i.cmm_updated ${adjusted_model}, strata(age_strata region_code) cformat(%9.2f)
	mat mat_hr = r(table)				
...
foreach l of local ex_level{
	...
	local label_name: label(`exposure') `l'
	lincom `l'.`exposure' + `l'.`exposure '#1.cmm_updated,eform
...
foreach l of local ex_level{
	...
	sum py if `exposure' == `l' & cmm_updated == 0
	local py_sum = r(sum)
	sum _d if `exposure' == `l' & cmm_updated == 0
...
foreach l of local ex_level{
	...
	local level_label: label(`exposure') `l'
	sum py if `exposure' == `l' & cmm_updated == 1
	local py_sum = r(sum)
	sum _d if `exposure' == `l' & cmm_updated == 1
	local ep_count = r(sum)
	local ir = string(`ep_count'/`py_sum'*10000,"%9.2f")
	post `IR' ("`level_label'")  (`ep_count') ("`ir'")
}
...
export delim using "2.Result/HRs//$S_DATE//`ep' `exposure' Updated_newduration $S_DATE.csv", replace
